...
* promiseType - If set to `native` all methods returning a promise will return a native JavaScript promise. If not set will return Keycloak specific promise objects. Since native JavaScript promises have become the industry standard it is recommended that you always specify `native`.
